...
**この節と §4.4・§6・§7.1・§7.4・§8・§11 の記述はまだ食い違っている。**
書き換えは B4 に着手する前の最初の作業として残っている（後述の「B4 に入る前にやること」）。
...
### B4 に入る前にやること（2026-08-09 の見直しで発生）

§2.3 で範囲が変わったのに、本文の各所がまだ旧構成のまま。**B4 の実装に入る前に、
まずこの書き換えを済ませること。** 中途半端に残すと、次の実装者が
「検索アーカイブを作る前提」で §6 を読む。

1. **消す** — §4.4（検索アーカイブのデータモデル）、§7.4（`archive.html`）、
   §8（変換スクリプト）、§2.1 と §2.2 の `archive.html` / `archive.enc` /
   `tools/build-archive.mjs` / `private/` の行
2. **§6 を組み直す** — §6.1 は「合言葉の二重の役割」を書いているが、①目隠しは
   静的サイトでは成立しない（`events.json` を直接叩けば読める）。②の復号鍵だけが残る。
   暗号化の対象を「検索データ」から「同期する JSON 全部」に書き換える
3. **§6.4 を §6 の外へ出す** — 中身は Leaflet をセルフホストする理由、つまり
   **公開用トークンの話**で、認証とも暗号化とも関係がない。§6 をまとめて畳むと
   CDN を使わない理由が消える。§5（保存と共有）の末尾あたりへ移す
4. **§11 の 2 行を直す** — 「Gmail / LINE の保護」の行は前提ごと無くなった。
   「旅程・持ち物の保護＝平文のまま」は決定が覆っている（暗号化する）
5. **§7.1 を直す** — 合言葉入力は残るが、メニューの「データ検索」カードは消える

これを終えてから B4 の実装計画を書く。**B4 は B1 の公開フローに手を入れる**
