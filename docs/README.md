...
`_config.yml` の `exclude` がこのディレクトリを Jekyll から外しているので、
**中に何を書いてもデプロイは壊れない**（波括弧 2 つも安全）。

**ただし配信は止まらない。** `exclude` は Jekyll に読ませないだけで、
`.nojekyll` があるいまは Jekyll 自体が動かず、リポジトリの中身がそのまま配信される。
`https://y-shinozaki.github.io/travel-plans/docs/README.md` は実際に 200 を返す。
リポジトリも public なので、**ここに置いたものは誰でも読める。**
