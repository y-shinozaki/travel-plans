...
**不要にしても `a` / `b` の値は消さない。** チェック → 不要のときは値を保持する
── 「間違えて不要にした」を無傷で取り消せるようにするため。チェックを
消してしまうと、取り消しても情報が戻らない。

ただし**不要を抜けてブランクへ戻るときはチェックも外す**（後述「状態の遷移」）。
3 段階を 1 つのボタンで回す形では「不要の次はブランク」が見た目の約束になるので、
値もそこで揃える。保持したままだと「ブランクに見えるのにチェック済み」という、
画面からは見えない食い違いが進捗の数字にだけ出る。値が無傷で残るのは
「チェック → 不要」の 1 区間だけで、「不要 → ブランク」では外れる。
...
> **注記（2026-08-11 追記）:** この計画は最初の（誤った）形のまま残している
> （CLAUDE.md の規約により、完了後の計画は歴史的記録であって正ではない）。
> **正しい形は前半の設計 ── とくに「描画」「状態の遷移」の節 ── を見ること。**
> とくに Task 3 のコード片（`naCell()` / `naMark()` / `.napill` / `.napill--off` /
> `.pkitem__nadash` / `.pkitem__na`、「通常モードはチェックボックス、編集モードは
> 『不要にする』のピル」という形）は使われていない。実装では 1 つのボタンが
> ブランク → チェック → 不要 → ブランクの 3 段階を回す形（`cycleCell()` /
> `.pkcycle` / `.pkcycle__box`）に作り直した。Task 1・2・4・5（検証・進捗・配線・
> ドキュメント）の方針はそのまま有効。
