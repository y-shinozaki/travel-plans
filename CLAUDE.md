...
**`_config.yml` と `.nojekyll` をどちらも消さないこと。** GitHub Pages は既定で
リポジトリ全体を Jekyll に通す。このサイトは素の静的ファイルで Jekyll の機能を
1 つも使っていないので、処理されて得るものが無い一方、失うものがある:

- **Markdown が Liquid テンプレートとして解釈される。** 2026-08-10、実装計画に書いた
  JSDoc の型注記（波括弧 2 つで始まる形）を Liquid の変数の開始と読んで構文エラーになり、
  **サイト全体のデプロイが 3 回続けて失敗した**。本番は前の版で止まり、その間
