 ...
 *
 * **a / b の値はここでは触らない。** 不要にした間はチェックが無傷で残るように
 * するため ── 消してしまうと「間違えて不要にした」を取り消しても情報が戻らない。
 * ただし、不要を抜けて完全なブランクに戻すのは cycleMember() の役目で、
 * そちらは戻すときに `[member]: false` も一緒に立てる（3 段階の最後の遷移
 * だけ値も揃える。plans/packing-not-applicable.md「保持と復帰」）。この関数
 * 単体を notNeeded: false で呼んでも、a / b の値は変えない。
 *
