...
## 状態: 2026-08-10 完了（Task 1〜8）。残っているのは実機での開通確認だけ

**このファイルは実行済みの計画で、これから実行するものではない。** 記録として残してある。

| | |
|---|---|
| Task 1〜7 | 完了。main にマージ済み（`9a59d0f`） |
| Task 8（本番の切り替え） | 実施済み（`9a8b90f`）。**Step 2 と Step 6 を除く** ── 詳細は Task 8 の「実施の記録」 |
| テスト | 373 件 pass / 0 fail |
| 本番 | `assets/data/events.json` は封筒 JSON。合言葉が無いと旅程を開けない |

**次のセッションへの引き継ぎ事項は 2 件**（どちらも人の作業）:

1. **6 名（依田家 3 名＋篠崎家 2 名＋恵美さん）の実機で開通確認**。一人ずつ、
   実際のスマートフォンで合言葉を入れ、旅程が見えること・タブを閉じて開き直しても
   再入力を求められないことを確かめる。**出発は 2026-08-12**
2. **PBKDF2 の所要時間を実機で確認**（Task 8 Step 2 を飛ばして切り替えた）。
   「開く」を押してから数秒待たされるようなら、反復回数を下げる判断が要る ──
   **ただし下げるには切り替えをやり直すことになる**

開けない人が出たときの切り分けは画面の文言がそのまま示す:
「合言葉が違います。」／「データが壊れています」／「この端末では鍵を保存できません」。

実行中の作業記録（SDD のレジャー）は worktree の中にあり、worktree ごと削除した。
**残す価値のある内容は下の「実行中に見つかったこと」に写してある。**

---

## 実行中に見つかったこと（引き継ぎ用の記録）

### この計画自身の欠陥（レビューが見つけ、実行中に計画を書き換えた）

いずれも「計画どおりに実装すると目的を達成しない」種類のもの。同じ設計に触るときの参考に。

1. **`createPublishUI()` を `load()` より前に移すだけでは何も変わらなかった。**
   この関数は要素を組み立てるだけで DOM に挿入せず、挿入するのは `start()` の中の
   `replaceChildren` だけ。その `start()` は `load()` の後ろに残す設計だったので、
   リモートが壊れた端末では公開ボタンもトークン設定も現れないままだった ──
   Task 5 が存在する理由そのものが未達。マウントを `createPublishUI()` 本体へ移した
2. **公開ボタンが出ても押せなかった。** `sync.load()` が投げると
   `setData(loaded.data)` に到達せず `state.data` が `null` のままで、
   `doPublish` が `validateEvents(null)` で必ず失敗する。しかもリモートの
   `events.json` が検証に落ちると `load()` は手元の下書きがどれだけ正しくても
   全端末で投げるので、**どの端末も UI から復旧できない**。`sync.readDraft()` を足し、
   `schedule.js` の catch が再送出の前に下書きを `state` へ載せるようにした
3. **合言葉の打ち間違いが「データが壊れています」と表示されていた。** ソルトは
   3 つの JSON で共有するので、間違った合言葉から作った鍵でも封筒の `kdf` は一致する。
   `crypto.js` の `kdf` 比較が捕まえられるのは「別のソルトで暗号化されている」場合だけ。
   **いちばん起きやすい打ち間違いは入力直後にしか捕まえられない** ──
   `auth-form.js` が `unlock` の直後に実際に `decode()` して確かめる形にした
4. **鍵が保存済みだと合言葉を入れ直す場所が無かった。** `hasKey` でフォームを隠す
   一方、`load-error.js` は「index.html に戻って入れ直してください」と案内していた。
   到達経路は 2 つ ── fetch 失敗時に検証なしの鍵を保存する経路と、公開端末での
   合言葉変更。「入れ直す」導線を `hasKey` に依存せず常設し（2 度押し）、
   fetch 失敗時は鍵を保存しないようにした

### 繰り越した Minor（次に触るときに triage すること）

- `tests/crypto.test.js` が `malformed` の分岐（不正 base64 / 復号後が JSON でない）を
  検査していない
- `crypto.js` が「`ct` はあるが `kdf` ごと壊れている」封筒を `malformed` ではなく
  `wrong-key` と報告する
- `adoptRemote()` が `outerStampMismatch` を捨てる（`load()` は出すのに取り込み経路は出さない）
- `saveLocal()` / `hasUnpublishedChanges()` を**注入した `draftKey`** で回すテストが無い。
  設計書が名指しする失敗（`saveLocal` が `tp:events` を上書きする）は `saveLocal` 経由なのに、
  そこを守るアサーションが無い。**B2 で 2 つ目の JSON を作るときに必ず足すこと**
- `tests/auth-form.test.js` の `dom()` に `els.status` を足したが、同ファイルの
  console 側のアサーションは両経路で自明に真のまま
- `auth-form.js` が `publish-ui.js` の module-private な `armedButton`（2 度押し）を
  再実装している。`dom.js` へ出せば重複が消える
- 「入れ直す」ボタンは 1 度押すと構えたまま戻らない（設計書 §13 の既知の負債と同じ形）

### 受け入れた残存リスク

- **保存領域に書けない端末は旅程を読めない。** B4 より前は読めた（`sync.js` の
  `load()` が `storeAdopted()` の失敗をわざと握っている）。文言だけ正確にし
  （「この端末では鍵を保存できません」）、鍵をメモリに保持する対応は見送った
- **壊れたリモートが「JSON としては読めて `updatedAt` が `base` より新しい」形だと
  409 で塞がる**（設計書 §13 に詳細）
- **実データの編集事故をコミット前に検知できなくなった**（設計書 §13
  「Phase B4 で失った検知能力」）
...
## Task 8: 本番を暗号文へ切り替え、開通を確認する — **2026-08-10 実施済み（一部を除く）**

> **実施の記録（引き継ぎ用）**
>
> | Step | 結果 |
> |---|---|
> | 1 合言葉を決める | 済。`yoda-shinozaki`。**値はここにも memory にも書かない**（雄一さんのパスワードマネージャ） |
> | 2 PBKDF2 を実機で測る | **未実施のまま切り替えた。** 開発機の Chrome で 196ms。実機は 0.6〜2 秒の見込みという判断で通した。**下げるには切り替えをやり直すことになる** |
> | 3 平文を控える | 済。切り替え前の main は `9a59d0f`、その 1 つ前の `events.json` が平文（`2c8d7ca` 時点の内容） |
> | 4 切り替える | 済。`9a8b90f`「Update itinerary from the browser (43 events)」。ブラウザの公開ボタンから |
> | 5 curl で確認 | 済。封筒 JSON が配信され、地名・人名は暗号文の外に出ていない |
> | 6 6 名の実機で開通確認 | **未実施。引き継ぎ事項** |
> | 7 実データのテストを更新 | 済。`59a4c43`。**計画が想定した `data.test.js` だけでなく、`categories` / `icons` / `event-editor` の 3 ファイルも実データを読んでいた** |
> | 8 commit + push | 済 |
>
> **合言葉の性質について記録**: 当初の案は「タイ」だったが、サイト自身が
> "Thailand" と「依田家・篠崎家 合同」を表示しているため最初の推測になると指摘し、
> `yoda-shinozaki` になった。**これもサイトの表示からローマ字化で導ける**ため、
> URL を知った人に対する防御としては弱い。その旨を伝えたうえでの本人の決定。
> **旅程に予約番号やパスポート番号を書き足さないこと。**
>
> **私（アシスタント）は合言葉もトークンもフィールドに入力できない**（安全上の規約）。
> Step 4 のうち「合言葉の設定」と「トークンの設定」は本人が実施し、
> 「公開」ボタンの押下と前後の検証をこちらで行った。
...
- [ ] **Step 6: 5 名＋恵美さんの端末で開通確認をする** — **未実施。引き継ぎ事項**
