...
**フォームには `image` / `imagePos` / `icon` の入力欄がない。** `image` と `imagePos` の
キーは実データ **40 件すべて**にあり（値が入っているのは `image` 24 件・`imagePos` 3 件）、
`icon` は 14 件にある。`readEventForm()` の戻り値でそのまま置き換えると、
**1 文字直しただけでキーごと消える。** しかもこれらは全部オプション項目なので
`validateEvents()` は通ってしまい、誰も気づかない。

既存イベントの編集では必ず併合すること:

```js
const updated = { ...original, ...readEventForm(getValue), id: original.id };
```

新規追加ではフォームの戻り値をそのまま使ってよい（元がないため）。

**この振る舞いをテストで押さえること。** `image` を持つイベントを読み込み、
タイトルだけ変えて保存したあと `image` が残っていることを確かめる。

### id の一意性はここで確かめる

`formProblems()` は 1 件しか見ないので `id` の重複を検出できない。採番したあと、
